...
const allStudents = computed(() => userStore.allStudents);
...
const fetchStudents = () => {
    api.userApi.fetch(ROLE_STUDENT).then((students) => {
        userStore.setStudents(students);
    }).catch((error) => {
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table class="w-full text-sm text-left">
                            ...
                            <tr v-for="student in allStudents">
                                ...
                                >
                                    <img class="w-10 h-10 rounded-full" :src="student?.avatar?? avatar" alt="Jese image">
                                    <div class="pl-3">
                                        <div class="text-base font-semibold">{{ getFullName(student) }}</div>
                                ...
                                >
                                    <span v-if="student.status" class="text-xs inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-green-500 text-white rounded-full">Activé</span>
                                    <span v-else class="text-xs inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-yellow-500 text-white rounded-full">En attente</span>
                                </td>
                                <td
