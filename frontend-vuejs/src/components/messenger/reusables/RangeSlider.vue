...
const modelValue = ref((props.initialValue) || 0);

// check if thumb is out of boundaries and move it to the location
const moveThumb = (location) => {
    ...
    thumb.value.style.left = location + 'px';
    fill.value.style.width = location + 'px';

    modelValue.value = (location / slider.value.clientWidth) * 100;
